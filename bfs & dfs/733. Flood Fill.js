...
var floodFill = function (image, sr, sc, newColor) {
    ...
    while (stack.length > 0) {
        let pos = stack.pop();
        if (pos[0] < 0 || pos[0] === w || pos[1] < 0 || pos[1] === h) continue; // out of image area
        const color = image[pos[0]][pos[1]];
        if (color === newColor || color !== originColor) continue;
        image[pos[0]][pos[1]] = newColor;
        for (const move of moves) {
            stack.push([pos[0] + move[0], pos[1] + move[1]]);
        }
    }
    return image;
};
